...
import { store, resetStore, resetFlush, flushState, subscribeFlush, encKey, chKey, setKey } from './store.js'
...
const untilState = (cid, id, want) =>
  new Promise((resolve) => {
    const k = encKey(cid, id)
    if (flushState(k) === want) return resolve()
    const un = subscribeFlush(() => { if (flushState(k) === want) { un(); resolve() } })
  })
...
test('edit marks the record unsaved; flush persists it and reports saved + onSaved(saved)', async () => {
  ...
  assert.equal(flushState(encKey('c1', 'e1')), 'unsaved')
  await store.encounters.flush('c1', 'e1')
  assert.equal(flushState(encKey('c1', 'e1')), 'saved')
...
test('a failed flush → error state + onError, keeps dirty, and recovers on the next edit', async () => {
  ...
  await store.encounters.flush('c1', 'e1')
  assert.equal(flushState(encKey('c1', 'e1')), 'error')
...
test('chapters.edit flushes through the store to the chapters endpoint (rtd8b-2)', async () => {
  const calls = stubFetch((url, o) => res({ ...o.body ? JSON.parse(o.body) : {}, id: 'ch1' }))
  let saved = null
  store.chapters.edit('c1', 'ch1', { name: 'Ch', order: 0, party_level: 3 }, { onSaved: (u) => { saved = u } })
  assert.equal(flushState(chKey('c1', 'ch1')), 'unsaved')
  await store.chapters.flush('c1', 'ch1')
  assert.equal(flushState(chKey('c1', 'ch1')), 'saved')
  const put = calls.find((c) => c.method === 'PUT')
  assert.match(put.url, /\/chapters\/ch1$/)
  assert.equal(put.body.name, 'Ch')
  assert.equal(saved.name, 'Ch')
})

test('settings.edit flushes through and writes the settings cache (rtd8b-2)', async () => {
  const calls = stubFetch((url, o) => (o.method === 'PUT' ? res(JSON.parse(o.body)) : res({})))
  store.settings.edit('c1', { party_level: 4, party_size: 5 }, {})
  await store.settings.flush('c1')
  assert.equal(flushState(setKey('c1')), 'saved')
  assert.match(calls.find((c) => c.method === 'PUT').url, /\/settings$/)
  const got = await store.settings.get('c1') // served from the write-through cache — no extra GET
  assert.deepEqual(got, { party_level: 4, party_size: 5 })
